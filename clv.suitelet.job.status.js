...
define(['N/https', 'N/search', "N/record", "N/log"], function (https, search, record, log) {
    ...
    function loadRecord(){
        ...
        data.push({
            ...
            subtotal : objRecord.getText({fieldId: "subtotal"}),
            taxtotal : objRecord.getText({fieldId: "taxtotal"}),
            handlingcost : objRecord.getText({fieldId: "handlingcost"}),
            ...
            shippingcost : objRecord.getText({fieldId: "shippingcost"}),
            total : objRecord.getText({fieldId: "total"}),
    ...
    function getOrderStatus(orderStatusVariables){
        ...
        if(orderStatusVariables[0].stock=="2"){status={text_status:"Stock Issues", value_status:"2"}; return status;};
        if(orderStatusVariables[0].artwork_setup=="4"){status={text_status:"Art Issues", value_status:"4"}; return status;};
        if(orderStatusVariables[0].approval_request=="2"){status={text_status:"Queued For Revision", value_status:"2"}; return status;};
        if(orderStatusVariables[0].approval_request=="3"){status={text_status:"Pending Response", value_status:"3"}; return status;};
        if(orderStatusVariables[0].payment=="5"){status={text_status:"Pending Payment Response", value_status:"5"}; return status;};
        ...
        {status={text_status:"Queued For Printing", value_status:"12"}; return status;};
        return status;
    };
